  ...
  <section>
  	<table class="boardlist">
  		<tr>
				<td colspan="5" style="border: white; text-align: right"><a
					href="ik.do?command=notice_writeForm">글 등록</a></td>
		</tr>
		<tr>
			<th>번호</th>
			<th>제목</th>
  	...
  	<table>
  		<tr>
  			...
  			%>
  			<td><a href="Superser?command=noticeboardList&page=1">[처음으로]</a><td>
  			<%if(block>1){ %>
  				<td><a href="Superser?command=noticeboardList&page=<%=prev_page %>">[이전]</a></td>
  			<%} %>
  			<%for(int i=pageStart; i<=pageEnd ;i++){ 
  				if(nowpage==i){%>
  					<td><p>[<%=i %>]</p></td>
  				<%} else{ %>
  					<td><a href="Superser?command=noticeboardList&page=<%=i %>">[<%=i %>]</a></td>
  				<%} %>
  			<%} %>
  			<%if(block<blockCount){ %>
  				<td><a href="Superser?command=noticeboardList&page=<%=next_page %>">다음</a></td>
  			<%} %>
  			<td><a href="Superser?command=noticeboardList&page=<%=pageCount %>">[맨뒤로]</a></td>
  		</tr>
  	</table>
