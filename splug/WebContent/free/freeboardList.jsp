  ...
  <section>
  	<table class="boardlist">
  		<tr>
				<td colspan="5" style="border: white; text-align: right"><a
					href="ik.do?command=free_writeForm">글 등록</a></td>
		</tr>
		<tr>
			<th>번호</th>
			<th>제목</th>
  	...
  	<table>
  		<tr>
  			...
  			%>
  			<td><a href="Superser?command=freeboardList&page=1">[처음으로]</a><td>
  			<%if(block>1){ %>
  				<td><a href="Superser?command=freeboardList&page=<%=prev_page %>">[이전]</a></td>
  			<%} %>
  			<%for(int i=pageStart; i<=pageEnd ;i++){ 
  				if(nowpage==i){%>
  					<td><p>[<%=i %>]</p></td>
  				<%} else{ %>
  					<td><a href="Superser?command=freeboardList&page=<%=i %>">[<%=i %>]</a></td>
  				<%} %>
  			<%} %>
  			<%if(block<blockCount){ %>
  				<td><a href="Superser?command=freeboardList&page=<%=next_page %>">다음</a></td>
  			<%} %>
  			<td><a href="Superser?command=freeboardList&page=<%=pageCount %>">[맨뒤로]</a></td>
  		</tr>
  	</table>
  	<form method="post" action="ik.do">
  		<input type="hidden" name="command" value="free_search">
  		<table>
  			<tr>
  				<td>
