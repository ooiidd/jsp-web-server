  ...
  <section>
  	<table class="boardlist">
  		<tr>
				<td colspan="5" style="border: white; text-align: right"><a
					href="Superser?command=ik_writeForm">글쓰기 등록</a></td>
		</tr>
		<tr>
			<th>번호</th>
			<th>제목</th>
		...
		<c:forEach var="board" items="${boardList }" varStatus="status">
			<tr class="record">
				<td>${board.num }</td>
				<td><a href="Superser?command=ik_show&num=${board.num}"> ${board.title } </a>
  	...
  	<table>
  		<tr>
  			...
  			%>
  			<td><a href="Superser?command=ikboardList&page=1">[처음으로]</a><td>
  			<%if(block>1){ %>
  				<td><a href="Superser?command=ikboardList&page=<%=prev_page %>">[이전]</a></td>
  			<%} %>
  			<%for(int i=pageStart; i<=pageEnd ;i++){ 
  				if(nowpage==i){%>
  					<td><p>[<%=i %>]</p></td>
  				<%} else{ %>
  					<td><a href="Superser?command=ikboardList&page=<%=i %>">[<%=i %>]</a></td>
  				<%} %>
  			<%} %>
  			<%if(block<blockCount){ %>
  				<td><a href="Superser?command=ikboardList&page=<%=next_page %>">다음</a></td>
  			<%} %>
  			<td><a href="Superser?command=ikboardList&page=<%=pageCount %>">[맨뒤로]</a></td>
  		</tr>
  	</table>
  	<form method="post" action="ik.do">
  		<input type="hidden" name="command" value="ik_search">
  		<table>
  			<tr>
  				<td>
