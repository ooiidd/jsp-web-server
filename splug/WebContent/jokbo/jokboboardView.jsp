  ...
  <section>
  	<div id="wrap" align="center">
		<table>
			...
			<c:forEach var="user" items="${useritem }" varStatus="status">
				<tr><td colspan="6"><a href="ik.do?command=jokboboard_down&file=${user}&serfile=${item[status.index]}">${user }
				</a></td></tr>
			</c:forEach>
		</table>
		<br> <br> 
		...
		<input type="button" value="게시글 수정"
			onclick="location.href='ik.do?command=jokboboard_update_form&num=${board.num}'">
		<form name="deleteform" action="ik.do" method="post">
			<input type="hidden" name="command" value="jokboboard_delete">
