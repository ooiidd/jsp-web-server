...
public class meetingBoardUpdateAction implements Action{
	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		BoardVO dVo=bDao.selectOneBoardByNum(str, "meetingboard");
		...
		if(dVo.getUserid().equals(login.getUserid()) ){
			bDao.updateIkBoard(bVo, "meetingboard");
			response.sendRedirect("/ik.do?command=meetingboardList&page=1");
			return;
		} else if(login==null){
			out.println("<script>"); 
