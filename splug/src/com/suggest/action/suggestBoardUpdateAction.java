...
public class suggestBoardUpdateAction implements Action{
	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		BoardVO dVo=bDao.selectOneBoardByNum(str, "suggestboard");
		...
		if(dVo.getUserid().equals(login.getUserid()) ){
			bDao.updateIkBoard(bVo, "suggestboard");
			response.sendRedirect("/splug/ik.do?command=suggestboardList&page=1");
			return;
		} else if(login==null){
			out.println("<script>"); 
